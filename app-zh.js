...
scripts(){ return ScriptManager.page('zh'); },
production(){ return productionDashboardZh(); },
...
analytics(){ return `${section('发布与数据','每集发布后统一记录，找出真正有效的内容模式',`<div class="grid grid-4">${['播放量','首30秒留存率','1分钟留存率','平均观看时长','完播率','追集率','新增粉丝','最受欢迎评论'].map((x,i)=>statCard(x,'—','0%')).join('')}</div>`)}${section('复盘问题','数据不是为了汇报，而是为了决定下一集拍什么',`<div class="grid grid-2">${card('内容表现','<ul class="list"><li>哪种新加坡文化最受欢迎？</li><li>哪种冲突最好笑？</li><li>哪些题材值得拍续集？</li></ul>')}${card('人物表现','<ul class="list"><li>哪个角色最受欢迎？</li><li>观众最喜欢哪组人物关系？</li><li>哪个平台效果最好？</li></ul>')}</div>`)}`; },
