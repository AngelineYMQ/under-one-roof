...
const availabilitySeedZh=[
 {member:'Angeline',dayOfWeek:2,startTime:'10:00',endTime:'14:00',note:'上午与午餐时段'},
 {member:'James',dayOfWeek:2,startTime:'11:00',endTime:'16:00',note:'下午前可拍'},
 {member:'Joseph',dayOfWeek:2,startTime:'12:00',endTime:'18:00',note:'午后较方便'},
 {member:'Angeline',dayOfWeek:6,startTime:'10:00',endTime:'17:00',note:'周六优先'},
 {member:'James',dayOfWeek:6,startTime:'09:00',endTime:'15:00',note:''},
 {member:'Joseph',dayOfWeek:6,startTime:'11:00',endTime:'19:00',note:''}
];
const weekZh=['周一','周二','周三','周四','周五','周六','周日'];
const memberClassZh={Angeline:'av-angeline',James:'av-james',Joseph:'av-joseph'};
function availabilityPageZh(){setTimeout(initAvailabilityZh,0);return `${section('三人共同时间','记录 Angeline、James 与 Joseph 每周固定有空的日期和时间，系统会自动找出三个人都方便的共同空档。',`<div class="availability-head"><div><span class="availability-kicker">团队共享排期</span><p>先登记各自时间，再从共同空档安排拍摄。</p></div><button class="primary-btn" onclick="openAvailabilityZh()">＋ 添加有空时间</button></div><div class="availability-tabs"><button class="active" onclick="setAvailabilityViewZh('overlap',this)">共同空档</button><button onclick="setAvailabilityViewZh('people',this)">按成员查看</button><button onclick="setAvailabilityViewZh('week',this)">每周总览</button></div><div id="availabilityViewZh"><div class="empty">正在载入团队时间…</div></div>`)}`;}
...
function commonSlotsZh(){const out=[];for(let d=1;d<=7;d++){const groups=['Angeline','James','Joseph'].map(m=>availabilityZh.filter(x=>x.member===m&&Number(x.dayOfWeek)===d));if(groups.some(g=>!g.length))continue;for(const a of groups[0])for(const j of groups[1])for(const o of groups[2]){const s=Math.max(toMinZh(a.startTime),toMinZh(j.startTime),toMinZh(o.startTime)),e=Math.min(toMinZh(a.endTime),toMinZh(j.endTime),toMinZh(o.endTime));if(e-s>=60)out.push({dayOfWeek:d,startTime:fmtMinZh(s),endTime:fmtMinZh(e),minutes:e-s})}}return out.sort((a,b)=>a.dayOfWeek-b.dayOfWeek||a.startTime.localeCompare(b.startTime));}
...
function renderAvailabilityOverlapZh(){const slots=commonSlotsZh();return `<div class="availability-summary"><div><span>已登记时间</span><strong>${availabilityZh.length}</strong></div><div><span>本周共同空档</span><strong>${slots.length}</strong></div><div><span>参与成员</span><strong>3 / 3</strong></div></div>${slots.length?`<div class="common-slot-grid">${slots.map((x,i)=>`<article class="common-slot-card"><div class="common-slot-day"><span>${weekZh[x.dayOfWeek-1]}</span><strong>${x.startTime}–${x.endTime}</strong></div><p>三个人都方便 · ${Math.round(x.minutes/60*10)/10} 小时</p><div class="common-members"><span class="av-angeline">A</span><span class="av-james">J</span><span class="av-joseph">J</span></div><button class="mini-btn" onclick="useCommonSlotZh(${i})">用这个时间安排拍摄</button></article>`).join('')}</div>`:`<div class="availability-empty"><strong>暂时没有三人共同空档</strong><p>继续添加或调整各自的有空时间，系统会自动计算重叠时段。</p></div>`}`;}
function renderAvailabilityPeopleZh(){return `<div class="availability-people">${['Angeline','James','Joseph'].map(m=>`<article class="availability-person"><header><div class="member-avatar ${memberClassZh[m]}">${m[0]}</div><div><strong>${m}</strong><span>${availabilityZh.filter(x=>x.member===m).length} 个时间段</span></div><button class="mini-btn" onclick="openAvailabilityZh('', '${m}')">＋ 添加</button></header><div class="availability-list">${availabilityZh.filter(x=>x.member===m).sort((a,b)=>a.dayOfWeek-b.dayOfWeek||a.startTime.localeCompare(b.startTime)).map(x=>`<button onclick="openAvailabilityZh('${x.id}')"><b>${weekZh[x.dayOfWeek-1]}</b><span>${x.startTime}–${x.endTime}</span><small>${x.note||'点击编辑'}</small></button>`).join('')||'<p class="empty-inline">尚未登记</p>'}</div></article>`).join('')}</div>`;}
function renderAvailabilityWeekZh(){return `<div class="availability-week">${weekZh.map((d,i)=>{const a=availabilityZh.filter(x=>Number(x.dayOfWeek)===i+1).sort((x,y)=>x.startTime.localeCompare(y.startTime));return `<article><header><strong>${d}</strong><span>${a.length} 段</span></header><div>${a.map(x=>`<button class="${memberClassZh[x.member]}" onclick="openAvailabilityZh('${x.id}')"><b>${x.member}</b><span>${x.startTime}–${x.endTime}</span></button>`).join('')||'<small>暂无时间</small>'}</div></article>`}).join('')}</div>`;}
function availabilityFormZh(x={}){return `<form id="availabilityFormZh" class="form-grid"><input type="hidden" name="id" value="${x.id||''}"><label><span>成员</span><select name="member">${['Angeline','James','Joseph'].map(m=>`<option ${x.member===m?'selected':''}>${m}</option>`).join('')}</select></label><label><span>星期</span><select name="dayOfWeek">${weekZh.map((d,i)=>`<option value="${i+1}" ${Number(x.dayOfWeek)===i+1?'selected':''}>${d}</option>`).join('')}</select></label><label><span>开始时间</span><input type="time" name="startTime" value="${x.startTime||'10:00'}" required></label><label><span>结束时间</span><input type="time" name="endTime" value="${x.endTime||'14:00'}" required></label><label class="full"><span>备注</span><input name="note" value="${x.note||''}" placeholder="例如：需要提前一天确认"></label><div class="modal-actions full">${x.id?'<button type="button" class="danger-btn" onclick="deleteAvailabilityZh()">删除</button>':''}<span class="modal-spacer"></span><button type="button" class="ghost-btn" onclick="closeAvailabilityModalZh()">取消</button><button class="primary-btn">保存时间</button></div></form>`;}
