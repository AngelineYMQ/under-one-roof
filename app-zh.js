...
[24,"Joseph 想搬走","关系升级篇"],[25,"Angeline 想把客厅改成办公室","关系升级篇"],[26,"James 突然要涨房租","关系升级篇"],[27,"三个人第一次一起过节","关系升级篇"],[28,"房东生病了","关系升级篇"],[29,"新租客来看房","关系升级篇"],[30,"到底谁才是一家之主","关系升级篇"]
];
...
let state = {
  ...
  ideas: JSON.parse(localStorage.getItem('roofIdeas') || JSON.stringify(defaultIdeas))
};
...
const pages = {
home(){
 return `<div class="hero"><p class="eyebrow">Season 1 · Internal HQ</p><h3>三个住在同一屋檐下的人，用喜剧演出中国人来到新加坡后会遇到的真实文化冲击。</h3><p>James 是房东，Angeline 与 Joseph 是租客。项目以 James 家客厅为核心场景，通过固定人物关系、生活冲突和真实的新加坡文化，建立可长期发展的短剧宇宙。</p><div class="hero-actions"><button class="primary-btn" onclick="location.hash='season'">查看第一季30集</button><button class="ghost-btn" onclick="location.hash='characters'">查看人物设定</button><button class="ghost-btn" onclick="openIdeaModal()">新增剧情灵感</button></div></div>
 ...
 ${section('项目核心','所有团队成员必须首先理解的四件事',`<div class="grid grid-2">${card('项目简介','<p>James 是房东，Angeline 和 Joseph 是租客。三个性格不同的人住在新加坡同一个屋檐下，通过日常冲突，带出新加坡生活、文化和社会规则。</p>')}${card('核心定位','<div class="callout">用轻松短剧，让中国观众看懂真实的新加坡。</div>')}${card('情感核心','<p>表面互相嫌弃，实际已经像一家人。喜剧来自冲突，长期追看来自人物之间越来越深的关系。</p>')}${card('扩展方向','<p>前期以客厅为主要舞台；后期可加入厨房、门口、楼下、小贩中心、超市、MRT、巴士站及更多固定配角。</p>')}</div>`)}
...
positioning(){ return introPage('项目定位','这不是普通搞笑账号，而是一套可以持续扩展的短剧 IP。',[
 ...
 ['一句话介绍','两个租客搬进新加坡房东的家，在鸡飞狗跳的合租生活中，逐渐了解新加坡。'],
 ['核心观众','对新加坡感兴趣的中国观众；准备来新加坡的人；已在新加坡生活的中国人；新加坡本地华人；喜欢生活类轻喜剧的观众。'],
 ['内容特点','新加坡真实文化、中国人与新加坡人的文化差异、合租生活、轻喜剧、实用信息、固定人物关系、连续世界观。']
], rules()); },
world(){ return introPage('世界观设定','客厅只是舞台，人物关系才是真正的核心。',[
 ['故事背景','James 在新加坡拥有或租住一套房子，并将房间出租给 Angeline 和 Joseph。三个人因为生活习惯、文化背景和性格不同，经常发生冲突。James 熟悉新加坡规则，Angeline 与 Joseph 经常因不了解或挑战规则而制造问题。'],
 ['固定场景','客厅、沙发、饭桌、厨房、房间门口、大门、走廊、楼下、电梯、附近小贩中心、超市、巴士站。前期主要拍 James 家客厅，后期逐步增加外景。'],
 ['世界观规则','James 是房东；Angeline 和 Joseph 是长期租客；三个人也是朋友；所有人物可以有工作和自己的生活；房子是主要场景但故事不局限于房子；配角可通过上门、电话、视频通话或外景出现。'],
 ['长期扩展','未来可进入找工作、创业、恋爱、家庭、教育、节日、政策、邻里关系等主题，同时保持三人关系不变。']
]); },
characters(){ return `${section('核心人物','人物设定必须稳定，但每个人都要有成长和弱点',`<div class="grid grid-3">${character('James','房东','J','avatar-james',['讲规则','理性','冷静','有一点固执','表面嫌弃租客，实际会帮他们'],'代表新加坡规则；解释文化；阻止租客做荒唐事；最后收拾残局。',['不可以永远像老师一样说教','不可以每集都发脾气','不可以把新加坡人演成冷漠的人','要有自己的弱点和笑点'])}${character('Angeline','租客','A','avatar-angeline',['反应快','有主见','什么都想变成机会','喜欢掌控局面','不愿意吃亏'],'推动剧情；制造冲突；挑战 James 的规则；把普通事情变成生意或大计划。',['不可以只演精明和强势','需要偶尔吃亏','需要有可爱和真实的一面','不可以永远都是正确的'])}${character('Joseph','租客','J','avatar-joseph',['好奇','天真','容易相信别人','想法奇怪','偶尔自作聪明'],'代表第一次来新加坡的人；提出观众想问的问题；误解文化；制造意外笑点。',['不可以永远像笨蛋','要有自己的聪明点','偶尔应该反过来解决问题','不能每集都被欺负'])}</div>`)}
 ${section('角色常见反应与口头表达','用于写剧本时快速保持人物一致',`<div class="grid grid-3">${card('James','<ul class="list"><li>强调规则与罚款</li><li>“这里是新加坡。”</li><li>无语地看着两位租客</li><li>最后默默帮忙解决</li></ul>')}${card('Angeline','<ul class="list"><li>“为什么不可以？”</li><li>“这样不是更有效率吗？”</li><li>“这个可以赚钱吗？”</li><li>先做了再通知别人</li></ul>')}${card('Joseph','<ul class="list"><li>“真的假的？”</li><li>“中国不是这样。”</li><li>“为什么？”</li><li>听懂一半就开始行动</li></ul>')}</div>`)}`; },
relations(){ return `${section('人物关系','固定关系决定每一集的冲突方式',`<div class="grid grid-2">${card('James × Angeline','<p><strong>规则制定者 vs 规则挑战者</strong></p><p>两个人经常争论，但彼此认可能力。</p>')}${card('James × Joseph','<p><strong>房东 vs 不断制造意外的租客</strong></p><p>James 经常需要帮 Joseph 收拾残局。</p>')}${card('Angeline × Joseph','<p><strong>主导者 vs 跟随者</strong></p><p>Angeline 经常带 Joseph 做事情，最后两个人一起闯祸。</p>')}${card('三人整体关系','<div class="callout"><strong>表面互相嫌弃，实际已经像一家人。</strong></div>')}</div>`)}
...
ideas(){ return `${section('剧情题材库','长期题材分类，不只服务第一季',`<div class="grid grid-3">${Object.entries(topicGroups).map(([k,v])=>card(k,`<div class="filters">${v.map(x=>badge(x)).join('')}</div>`)).join('')}</div>`)}${section('团队灵感箱','新增内容会保存在本机浏览器',`<div id="ideasTable">${ideasTable()}</div>`)}`; },
...
team(){ return `${section('核心团队','当前固定成员',`<div class="grid grid-3">${teamCard('James','房东／演员／场地主人','负责房东角色、场地协调、新加坡文化把关')}${teamCard('Angeline','租客／演员／项目策划','负责项目方向、剧情推动、市场定位与中国观众视角')}${teamCard('Joseph','租客／演员','负责观众视角、喜剧反应与文化误解型剧情')}</div>`)}${section('未来团队职位','新成员加入后可建立成员档案与权限',`<div class="grid grid-4">${['演员','编剧','导演','摄影','收音','剪辑','字幕','运营','商务','道具','服装','制片'].map(x=>card(x,'<p>可设为固定成员或项目制成员。</p>')).join('')}</div>`)}${section('成员档案字段','统一记录',`<div class="callout">姓名、照片、职位、联系方式、加入日期、是否固定成员、职责、查看权限、编辑权限、管理员权限。</div>`)}`; },
supporting(){ return `${section('配角与客串角色','配角应可重复出现，逐步建立完整宇宙',`<div class="grid grid-3">${['隔壁邻居','James 的妈妈','房屋中介','快递员','清洁阿姨','Angeline 的客户','Joseph 的朋友','新租客','ICA 工作人员','老板','同事','外卖员','小贩中心老板'].map(x=>card(x,'<p>记录人设、与主角关系、首次出现集数、可重复剧情和演员资料。</p>')).join('')}</div>`)}`; },
...
function ideasTable(){ if(!state.ideas.length) return '<div class="empty">暂无灵感</div>'; return `<div class="table-wrap"><table><thead><tr><th>标题</th><th>分类</th><th>一句话剧情</th><th>角色</th><th>场景</th><th>状态</th></tr></thead><tbody>${state.ideas.map(i=>`<tr><td><strong>${i.title}</strong></td><td>${i.category}</td><td>${i.summary}</td><td>${i.lead}</td><td>${i.scene}</td><td>${badge(i.status,i.status==='待讨论'?'yellow':'')}</td></tr>`).join('')}</tbody></table></div>`; }
...
document.getElementById('ideaForm').onsubmit=(e)=>{ e.preventDefault(); const d=Object.fromEntries(new FormData(e.target)); state.ideas.push({...d,status:'灵感箱'}); localStorage.setItem('roofIdeas',JSON.stringify(state.ideas)); e.target.reset(); modal.classList.remove('open'); if(state.page==='ideas') render(); };
document.getElementById('resetBtn').onclick=()=>{ localStorage.removeItem('roofIdeas'); state.ideas=JSON.parse(JSON.stringify(defaultIdeas)); render(); };
render();
