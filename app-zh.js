...
function schedulePageZh(){setTimeout(initScheduleZh,0);return `${section('日历与制作安排','统一记录拍摄、剪辑、约见和其他团队事项；所有成员共享同一份排期',`<div class="schedule-head"><div class="schedule-stats" id="scheduleStatsZh"></div><div class="schedule-head-actions"><button class="primary-btn" onclick="openCalendarEventZh()">＋ 新建日历事项</button></div></div><div class="schedule-toolbar"><div class="production-tabs"><button class="${scheduleViewModeZh==='cards'?'active':''}" onclick="setScheduleViewZh('cards',this)">事项卡片</button><button class="${scheduleViewModeZh==='calendar'?'active':''}" onclick="setScheduleViewZh('calendar',this)">日历</button><button class="${scheduleViewModeZh==='list'?'active':''}" onclick="setScheduleViewZh('list',this)">列表</button></div><select id="scheduleStatusZh" onchange="renderScheduleZh()"><option value="">全部状态</option>${Object.entries(shootStatusZh).map(([k,v])=>`<option value="${k}">${v}</option>`).join('')}</select><input id="scheduleSearchZh" placeholder="搜索日期、地点或集数" oninput="renderScheduleZh()"></div><div id="scheduleViewZh"><div class="empty">正在载入拍摄日程…</div></div>`)}`;}
...
function scheduleStatsZh(){const a=shootDaysZh,ep=new Set(a.flatMap(x=>(x.episodes.match(/EP\d+/g)||[]))),pending=a.filter(x=>x.status!=='completed').length,issues=a.filter(x=>x.issues||x.notes).length;return [['日历事项',a.length],['关联集数',ep.size],['待完成事项',pending],['有备注事项',issues]].map(([k,v])=>`<div><span>${k}</span><strong>${v}</strong></div>`).join('');}
...
function renderMonthCalendarZh(){const y=scheduleMonthZh.getFullYear(),m=scheduleMonthZh.getMonth(),first=new Date(y,m,1),days=new Date(y,m+1,0).getDate(),offset=first.getDay();const events=calendarEventsZh();let cells='';for(let i=0;i<offset;i++)cells+='<div class="month-cell outside"></div>';for(let d=1;d<=days;d++){const date=`${y}-${String(m+1).padStart(2,'0')}-${String(d).padStart(2,'0')}`,items=events.filter(x=>x.date===date).sort((a,b)=>(a.startTime||a.callTime||'').localeCompare(b.startTime||b.callTime||''));cells+=`<div class="month-cell"><button class="month-day-number" onclick="openCalendarEventZh('', '${date}')">${d}</button><div class="month-events">${items.map(x=>{const type=x.eventType||'other';const time=x.startTime||x.callTime||'';const labels={shoot:'拍摄',editing:'剪辑',meeting:'约见',other:'其他'};const label=x.title||x.episodes||labels[type];return `<button class="month-event ${type}" onclick="openCalendarEventZh('${x.id}')"><span>${time}</span><b>${label}</b><small>${x.assignee||x.owner||x.location||'未分配'}</small></button>`}).join('')}</div></div>`}return `<div class="month-calendar"><header><button class="month-nav" onclick="changeScheduleMonthZh(-1)">‹</button><h3>${calendarLabelZh()}</h3><button class="month-nav" onclick="changeScheduleMonthZh(1)">›</button></header><div class="month-weekdays">${['日','一','二','三','四','五','六'].map(x=>`<span>周${x}</span>`).join('')}</div><div class="month-grid">${cells}</div><footer><span><i class="legend-dot shoot"></i>拍摄</span><span><i class="legend-dot editing"></i>剪辑</span><span><i class="legend-dot meeting"></i>约见</span><span><i class="legend-dot other"></i>其他</span></footer></div>`;}
...
window.setScheduleViewZh=(v,b)=>{scheduleViewModeZh=v;const route=`schedule-${v}`;if(location.hash.replace('#','')!==route){location.hash=route;return;}document.querySelectorAll('.schedule-toolbar .production-tabs button').forEach(x=>x.classList.remove('active'));b?.classList.add('active');renderScheduleZh();};window.renderScheduleZh=()=>{const el=document.getElementById('scheduleViewZh');if(!el)return;document.getElementById('scheduleStatsZh').innerHTML=scheduleStatsZh();const a=filteredShootDaysZh();if(scheduleViewModeZh==='list')el.innerHTML=`<div class="table-wrap"><table><thead><tr><th>日期</th><th>类型</th><th>时间</th><th>事项／集数</th><th>负责人</th><th>状态</th><th></th></tr></thead><tbody>${a.map(x=>`<tr><td>${x.date}</td><td>${({shoot:'拍摄',editing:'剪辑',meeting:'约见／面谈',other:'其他'})[x.eventType]||'其他'}</td><td>${x.startTime||x.callTime||'—'}–${x.endTime||'—'}</td><td>${x.title||x.episodes||'—'}</td><td>${x.assignee||x.owner||'—'}</td><td>${shootStatusZh[x.status]||x.status}</td><td><button class="mini-btn" onclick="openCalendarEventZh('${x.id}')">查看</button></td></tr>`).join('')}</tbody></table></div>`;else if(scheduleViewModeZh==='calendar')el.innerHTML=renderMonthCalendarZh();else el.innerHTML=a.length?`<div class="calendar-item-grid">${a.map(x=>{const type=x.eventType||'other',typeLabel=({shoot:'拍摄',editing:'剪辑',meeting:'约见／面谈',other:'其他'})[type]||'其他',owner=x.assignee||x.owner||'未分配',time=`${x.startTime||x.callTime||'—'}–${x.endTime||'—'}`,title=x.title||x.episodes||'未命名事项';return `<article class="calendar-item-card type-${type}" onclick="openCalendarEventZh('${x.id}')"><header><div><span class="calendar-type-badge">${typeLabel}</span><strong>${x.date||'—'}</strong></div><span class="calendar-status">${shootStatusZh[x.status]||x.status||'筹备中'}</span></header><h4>${title}</h4><div class="calendar-item-meta"><div><small>时间</small><b>${time}</b></div><div><small>负责人</small><b>${owner}</b></div><div><small>相关集数</small><b>${x.episodes||'—'}</b></div></div>${x.location?`<p class="calendar-location">地点：${x.location}</p>`:''}${x.notes||x.issues?`<p class="calendar-note">${x.notes||x.issues}</p>`:''}<footer><span>点击查看或编辑</span><b>→</b></footer></article>`}).join('')}</div>`:'<div class="empty">暂无符合条件的日历事项</div>';};
function calendarEventFormZh(x={}){return `<form id="calendarEventFormZh" class="form-grid"><input type="hidden" name="id" value="${x.id||''}"><label><span>事项类型</span><select name="eventType" required><option value="shoot" ${x.eventType==='shoot'?'selected':''}>拍摄</option><option value="editing" ${x.eventType==='editing'?'selected':''}>剪辑</option><option value="meeting" ${x.eventType==='meeting'?'selected':''}>约见／面谈</option><option value="other" ${x.eventType==='other'||!x.eventType?'selected':''}>其他</option></select></label><label><span>负责人</span><select name="assignee"><option value="">未分配</option>${['Angeline','James','Joseph'].map(n=>`<option ${x.assignee===n||x.owner===n?'selected':''}>${n}</option>`).join('')}</select></label><label><span>日期</span><input type="date" name="date" value="${x.date||''}" required></label><label><span>开始时间</span><input type="time" name="startTime" value="${x.startTime||x.callTime||'10:00'}" required></label><label><span>结束时间</span><input type="time" name="endTime" value="${x.endTime||'12:00'}" required></label><label class="full"><span>事项</span><input name="title" value="${x.title||x.episodes||''}" placeholder="例如：拍摄 EP03／剪辑粗剪版／与演员面谈" required></label><label class="full"><span>相关集数</span><input name="episodes" value="${x.episodes||''}" placeholder="例如：EP03、EP04"></label><label class="full"><span>地点</span><input name="location" value="${x.location||''}" placeholder="例如：James 家／Zoom／办公室"></label><label class="full"><span>备注</span><textarea name="notes" rows="4">${x.notes||''}</textarea></label><input type="hidden" name="status" value="${x.status||'planning'}"><div class="modal-actions full">${x.id?'<button type="button" class="danger-btn" onclick="deleteCalendarEventZh()">删除</button>':''}<span class="modal-spacer"></span><button type="button" class="ghost-btn" onclick="closeScheduleModalZh()">取消</button><button class="primary-btn">保存事项</button></div></form>`;}
window.openCalendarEventZh=(id='',date='')=>{const x=shootDaysZh.find(i=>String(i.id)===String(id))||{eventType:'other',date};showScheduleModalZh(x.id?'编辑日历事项':'新建日历事项',calendarEventFormZh(x));setTimeout(()=>document.getElementById('calendarEventFormZh').onsubmit=saveCalendarEventZh,0);};async function saveCalendarEventZh(e){e.preventDefault();const d=Object.fromEntries(new FormData(e.target));if(d.endTime<=d.startTime){alert('结束时间必须晚于开始时间');return}let y=d.id?await SharedSchedules.update(d):await SharedSchedules.add(d);shootDaysZh=d.id?shootDaysZh.map(x=>String(x.id)===String(d.id)?y:x):[...shootDaysZh,y];closeScheduleModalZh();renderScheduleZh();}window.deleteCalendarEventZh=async()=>{const id=document.querySelector('#calendarEventFormZh [name=id]').value;if(confirm('确定删除这个日历事项？')){await SharedSchedules.remove(id);shootDaysZh=shootDaysZh.filter(x=>String(x.id)!==String(id));closeScheduleModalZh();renderScheduleZh();}};
